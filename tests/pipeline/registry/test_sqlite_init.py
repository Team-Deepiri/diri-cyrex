...
def test_columns_artifacts(conn):
    ...
    Note: ``created_at`` is intentionally omitted from the DDL per the
    design plan — it lives in the Pydantic model but is not stored as a
    separate column. The store's ``_bundle_to_row`` serializes it and
    ``_row_to_bundle`` falls back to ``datetime.now(timezone.utc)`` when
